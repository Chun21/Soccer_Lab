...
class _DummyController:
    def __init__(self):
        self.reset_called = 0
        self.infer_called = 0
        self.default_angles = torch.tensor([1.5, -0.5], dtype=torch.float32)
    ...
    def infer(self, **kwargs):
        self.infer_called += 1
        return torch.tensor([1.5, -0.5], dtype=torch.float32)
...
def test_motion_policy_is_updated_once_in_pre_physics_and_reused_in_apply_action():
    env_mod = _load_single_env_module()
    env = env_mod.SoccerSingleG1Env.__new__(env_mod.SoccerSingleG1Env)
    ...
    env._default_joint_pos = {"a1": torch.zeros((1, 5), dtype=torch.float32)}
    env._joint_pos_target = {"a1": torch.full((1, 5), -100.0, dtype=torch.float32)}
    env.robots = {"a1": _DummyRobot()}
    env.actions = {"a1": torch.tensor([[123.0, 456.0]], dtype=torch.float32)}
    env._policy_leg_joint_targets = {"a1": torch.zeros((1, 2), dtype=torch.float32)}
    env._motion_controller = _DummyController()

    env._pre_physics_step({"a1": torch.tensor([[7.0, 8.0]], dtype=torch.float32)})
    assert env._motion_controller.infer_called == 1
    torch.testing.assert_close(env._policy_leg_joint_targets["a1"], torch.tensor([[1.5, -0.5]]))

    env._apply_action()

    assert env._motion_controller.infer_called == 1
    assert env.robots["a1"].last_target is not None
...
def test_reset_motion_policy_calls_controller_reset_once():
    env_mod = _load_single_env_module()
    env = env_mod.SoccerSingleG1Env.__new__(env_mod.SoccerSingleG1Env)
    env.cfg = types.SimpleNamespace(use_unitree_rl_policy=True)
    env._motion_controller = _DummyController()
    ...
    assert env._motion_controller.reset_called == 1
    torch.testing.assert_close(env._policy_leg_joint_targets["a1"], torch.tensor([[1.5, -0.5]]))
